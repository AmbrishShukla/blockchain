...
void print(node *head)
{
    ...
    while (temp != NULL)
    {
        cout << (*temp).data << " " << temp->currdata << " " << temp->prevdata << endl;
...
node *takeinput()
{
    ...
    while (data != "-1")
    {
        string hashmsg = hashMessage(data);
        node *newnode = new node(data, hashmsg);
        if (head == NULL)
        {
            head = newnode;
            head->prevdata = "NULL";
        ...
        else
        {
            ...
            temp->next = newnode;
            newnode->prevdata = temp->currdata;
            newnode->prev = temp;
        }
        cin >> data;
    }
